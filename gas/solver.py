...
class GeometricAnnealingSolver:
    ...
    def __init__(
        self, lattice: "E8Lattice", energy_terms: List[EnergyTerm], params: GASParams
    ):
    ...
    def _compute_weights(self, rho: float) -> np.ndarray:
        ...
        w_rational = 0.5 * (1 - np.tanh(self.params.beta * (rho - self.params.rho_0)))

        # Exceptional family (phi): favor high rho
        w_exceptional = 0.5 * (
            1 + np.tanh(self.params.beta * (rho - self.params.rho_0))
        )
    ...
    def step(self, state: GASState) -> GASState:
        ...
        accept = delta_E <= 0 or np.random.rand() < np.exp(-delta_E / (T_t + 1e-10))
